# I copied the functionality messages from the WSJT-X source (NetwordMessages.hpp)
# version 1.8.0 r8193.  I have also added comments and data dumps of the actual
# packets of data I used to verify that my understanding of said  packets were correct.
...
# Use the fake packets I put in a file.... this will be an actual packet.
# with open('WSJT-X_UDPTestData.bin', mode='rb') as file:    

### Log packets test... 
#with open('Logged.bin', mode='rb') as file:     
#    fileContent = file.read(0x69*3)                     # read 3 logged packets

# Heartbeat Test
#    fileContent = file.read(0x224)                      # a heart beatpacket starts at 0x224
#    fileContent = file.read(44)                    

# Status Packet Test
#    fileContent = file.read(0x66)                   

# Decode Packet Test
#    fileContent = file.read(0x66)                   
#    fileContent = file.read(68)                     
...
try:
   while True:
        fileContent, addr = sock.recvfrom(1024) # buffer size is 1024 bytes

        NewPacket = WSJTX_Packet(fileContent, 0)
        NewPacket.Decode()
        print ("\n********** New Packet ************")
        print ("MagicNumber    : ", '0x{0:04x}'.format(NewPacket.MagicNumber))
        print ("Schema Ver     : ", '0x{0:04x}'.format(NewPacket.SchemaVersion))
        print ("PacketType     : ", '0x{0:04x}'.format(NewPacket.PacketType))
        print ("Client ID      : ", NewPacket.ClientID)
        print ('\n')

        if NewPacket.PacketType == 0:
            print ("Heartbeat Message")
            HeartbeatPacket = WSJTX_Heartbeat(fileContent, NewPacket.index)
            HeartbeatPacket.Decode()
            print ("MaxSchema      : ", '0x{0:04x}'.format(HeartbeatPacket.MaximumSchema))
            print ("Version        : ", HeartbeatPacket.Version)
            print ("Revision       : ", HeartbeatPacket.Revision)

        elif NewPacket.PacketType == 1:
            print("Status Message")
            StatusPacket = WSJTX_Status(fileContent, NewPacket.index)
            StatusPacket.Decode()
            print ("Frequency      : ", '{0:09d}'.format(StatusPacket.Frequency))  
            print ("Mode           : ", StatusPacket.Mode)
            print ("DX Call        : ", StatusPacket.DXCall)
            print ("Report         : ", StatusPacket.Report)
            print ("Tx Mode        : ", StatusPacket.TxMode)
            print ("TxEnabled      : ", StatusPacket.TxEnabled)
            print ("Trmitting      : ", StatusPacket.Transmitting)
            print ("Decoding       : ", StatusPacket.Decoding)
            print ("RxDF           : ", '{0:09d}'.format(StatusPacket.RxDF))
            print ("TxDF           : ", '{0:09d}'.format(StatusPacket.TxDF))
            print ("DE Call        : ", StatusPacket.DECall)
            print ("DE Grid        : ", StatusPacket.DEgrid)
            print ("DX Grid        : ", StatusPacket.DXgrid)
            print ("Tx Watchdog    : ", StatusPacket.TxWatchdog)
            print ("Sub Mode       : ", StatusPacket.Submode)
            print ("Fast Mode      : ", StatusPacket.Fastmode)
    
        elif NewPacket.PacketType == 2:
            print ("Decode Message")
            DecodePacket = WSJTX_Decode(fileContent, NewPacket.index)
            DecodePacket.Decode()
            print ("New            : ", DecodePacket.New)
            print ("Time           : ", '{0:09d}'.format(DecodePacket.Time))
            print ("snr            : ", '{0:09d}'.format(DecodePacket.snr))
            print ("DeltaTime      : ", '{0:09f}'.format(DecodePacket.DeltaTime))
            print ("DeltaFrequency : ", '{0:09d}'.format(DecodePacket.DeltaFrequency))
            print ("Mode           : ", DecodePacket.Mode)
            print ("Message        : ", DecodePacket.Message)
            print ("Low Cofidence  : ", DecodePacket.LowConfidence)
            print ("Off Air        : ", DecodePacket.OffAir)

        elif NewPacket.PacketType == 3:
            print ("Clear Message (There is no data the message is the data!)")

        # This is an IN message for the client sent by the server... duh we are the server! 
        elif NewPacket.PacketType == 4:
            print ("Reply Message")
        
        elif NewPacket.PacketType == 5:
            print ("Logged Message")
            LoggedPacket = WSJTX_Logged(fileContent, NewPacket.index)
            LoggedPacket.Decode()
            print ("DateOff  : ", '{0:09d}'.format(LoggedPacket.DateOff))   # unsigned 32bit
            print ("TimeOff  : ", '{0:09d}'.format(LoggedPacket.TimeOff))
            print ("TOffSpec : ", '{0:09d}'.format(LoggedPacket.TimeOffSpec))
            print ("TOfOffset: ", '{0:09d}'.format(LoggedPacket.TimeOffOffset))
            print ("DX Call  : ", LoggedPacket.DXcall)
            print ("DX Grid  : ", LoggedPacket.DXgrid)
            print ("Frequency: ", '{0:09d}'.format(LoggedPacket.DialFrequency))
            print ("Mode     : ", LoggedPacket.Mode)
            print ("Sent     : ", LoggedPacket.ReportSent)  
            print ("Rcvd     : ", LoggedPacket.ReportReceived)
            print ("Tx Pwr   : ", LoggedPacket.TxPower)
            print ("Comments : ", LoggedPacket.Comments)
            print ("Name     : ", LoggedPacket.Name)
            print ("Date On  : ", '{0:09d}'.format(LoggedPacket.DateOn))
            print ("Time On  : ", '{0:09d}'.format(LoggedPacket.TimeOn))    
            print ("TOnSpec  : ", '{0:09d}'.format(LoggedPacket.TimeOnSpec))
            print ("TOnOffset: ", '{0:09d}'.format(LoggedPacket.TimeOnOffset)) 

        elif NewPacket.PacketType == 6:
            print ("Close Message (There is no data the message is the data!)")
            try:
                print("Closing socket...")
                sock.close()
                sys.exit()
            except OSError:
                print ("exception while closing socket!")
                sys.exit()
        # This is an IN message for the client sent by the server... duh we are the server! 
        elif NewPacket.PacketType == 7:     # Sent message should never receive this! Here for referance
            print ("Replay Message")

        # This is an IN message for the client sent by the server... duh we are the server! 
        elif NewPacket.PacketType == 8:     # Sent message should never receive this! Here for referance
            print ("Halt Tx Message")
        
        # This is an IN message for the client sent by the server... duh we are the server! 
        elif NewPacket.PacketType == 9:     # Sent message should never receive this! Here for referance
            print ("Free Text Message")

        elif NewPacket.PacketType == 10:    
            print ("WSPR Decode Message")
